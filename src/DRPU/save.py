...
class Results(object):
    ...
    def save(self, key, path, index=None):
        if index is None:
            np.savetxt(
                os.path.join(path, "{}.csv".format(key)),
                np.array(self.map[key]),
                encoding="utf-8",
            )
        else:
            np.savetxt(
                os.path.join(path, "{}-{}.csv".format(key, index)),
                np.array(self.map[key]),
                encoding="utf-8",
            )
    ...
    def plot(self, keys, path, xlabel="Epoch", ylabel="Loss", marker=None):
        ...
        for key in keys:
            ax.plot(
                np.arange(1, 1 + len(self.map[key]), 1),
                np.array(self.map[key]),
                marker=marker,
                label=key,
            )
...
def output_test_results(
    path, test_idx, true_prior, acc, auc, prior=None, thresh=None, boundary=None
):
    with open(path, mode="a", encoding="utf-8") as f:
        if test_idx == 0:
            print("--Test result---\n", file=f)
